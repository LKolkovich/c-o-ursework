...
char* toBin(int num);
void huySentence(Text *text);
void huyWord(Word *word);
...
int main() {
    ...
    while(typeOfProgram != 6){
        ...
        wprintf(L"5 - сделать вот ту хуёвину для всего текста\n");
        wprintf(L"6 - Закрыть программу\n");
...
char* toBin(int num){
    char *doub = malloc(sizeof(char) * (int)log2(num) + 2);
    doub[1] = '\0';
    int len = 1;
    while(num){
        *doub = (char)(num % 2 + 48);
        num /= 2;
        len++;
        memmove(doub + sizeof(char), doub, len * sizeof(char));
    }
    memmove(doub, doub + sizeof(char), len * sizeof(char));
    return doub;
}

void huySentence(Text *text){
    for(int i = 0; i < text->sentenceCount; i++){
        for(int j = 0; j < text->sentence[i].wordCount; j++){
            huyWord(&text->sentence[i].words[j]);
        }
    }
}

void huyWord(Word *word){
    char* number = malloc(sizeof(char) * (word->len + 1));
    number[1] = '\0';
    int numberInt;
    int len = 1;
    char *binNumber;
    for(int i = 0; i < word->len; i++){
        if(isdigit(word->chars[i])){
            number[len - 1] = word->chars[i];
            len++;
        }

        if(((!isdigit(word->chars[i]) && (len != 1))) || ((i == word->len - 1) && (isdigit(word->chars[i])))){
        //if((!isdigit(word->chars[i]) || ((i == word->len - 1) && (isdigit(word->chars[i])))) && len != 1){
            numberInt = atoi(number);
            binNumber = toBin(numberInt);

            word->chars = realloc(word->chars, sizeof(char) * (word->len + strlen(binNumber)));
            if(!isdigit(word->chars[i])){
                memmove(word->chars + i - 2 * len + 2 + strlen(binNumber), word->chars + i - len + 1,
                        word->len - i + len);
                memcpy(word->chars + i - len + 1, binNumber, strlen(binNumber));
            }
            else{
                memmove(word->chars + i - 2 * len + 2 + strlen(binNumber), word->chars + i - len + 1,
                        word->len - i + len);
                memcpy(word->chars + i - len + 2, binNumber, strlen(binNumber));
            }
            i += (strlen(binNumber) - len);
            word->len += (strlen(binNumber) - len);
            len = 1;
        }
    }
}
